     ...
     $gender = array();
		...
		foreach ($doc->find('._groups-wrap ul li') as $ev ) {
		    ...
		    if(($ev->find('a:eq(0)')->attr('href'))!=""){ 
                ...
                $str = (string)$it->find('.price._product-price .line-through')->attr('data-price');
                $str = preg_replace("/[^0-9]/", '', $str);
                $price_bf[] = $str;
                $str = (string)$it->find('.price._product-price .sale')->attr('data-price');
                $str = preg_replace("/[^0-9]/", '', $str);
                $price_af[] = $str;
                $count++;
				}
		...
		for($i=0;$i<$count;$i++){
            ...
            array_push($p_brand,'Zara');
            array_push($p_category,'All');
            array_push($gender,"M");
